...
**Comandos.** O **alvo** se escolhe clicando nas próprias linhas das contas conectadas (a linha fica marcada e o nick aparece como chip ao lado de "Todas"); sem escolha, o comando vai para todas as online e não pausadas, e o toast diz em quantas agiu. Tudo se faz pela **barra de comando** (`src/components/CommandBar.tsx`): texto sem verbo é fala; texto começando com `:` é um comando de chat do servidor (sugeridos a partir do pacote 432); um verbo em português no começo vira o comando correspondente (`ir até Dominguez`, `fixar Dominguez`, `sussurrar Ana oi`, `entrar 7078217`, `sentar`, `convidar todos festa`…), com sugestões enquanto digita (Tab ou Enter aceita, setas escolhem) e histórico (seta para cima com a barra vazia). Abaixo dela, pílulas por grupo: as sem argumento executam na hora, as outras preenchem o verbo. O interpretador é puro e testado (`src/crowd/commandParser.ts`, `tests/commandParser.test.ts`) e cobre cada um dos comandos abaixo. Grupos:
...
**Pool de proxies.** A aba **Proxies** da Multidão é só o pool (`src/components/ProxyPoolView.tsx`): uma tabela com status, rótulo editável, host, IP de saída, quem usa (com as fotos) e ações, filtros (todos, livres, em uso, anti-VPN) e a coluna para colar a lista; tudo o mais de rede (sem tela, fila de conexão, proxy depois do login, proxy só para o jogo, tráfego, proxy próprio por conta, distribuir em lote, Tor, captcha) mora na aba **Rede**. Cole a lista inteira de proxies de uma vez (um por linha, no formato que o provedor exporta, `user:pass@host:porta`, ou com esquema; linhas com `#` são ignoradas; repetidos não entram duas vezes). Cada item fica criptografado em `crowd.json`, fora do perfil exportável. Na linha de cada conta aparece um **seletor**: escolha ali com qual proxy do pool a conta vai conectar (ou "proxy próprio"/"IP real"); vale na próxima conexão. O pool **aprende com o uso**, para você não testar um por um: conta que autentica marca o proxy como `ok`; site que recusa o login (o anti-VPN do Habblet) marca `anti-VPN` com a mensagem; falha para sair pelo proxy marca `erro`. Cada item mostra o IP de saída, quem já o usa e o motivo do status; dá para testar o IP de qualquer item sem conta ("testar" e "testar novos"), bloquear/liberar à mão, dar rótulo e remover. "Distribuir livres" dá um proxy ainda não usado (nem bloqueado, nem com erro) a cada conta sem proxy. Código: `electron/proxy-pool.ts` (leitura da lista e dedupe, testado em `tests/proxyPool.test.ts`), `electron/crowd-store.ts` (armazenamento, resolução do proxy efetivo por conta e IPC), `src/crowd/useCrowdController.ts` (o ouvinte de status que marca o pool).

**Proxy por conta.** Cada conta também pode ter um proxy próprio digitado direto (`socks5://user:pass@host:porta`, `http://host:porta`), aplicado à sessão dela antes do primeiro request: site e WebSocket do jogo saem por ele, o que resolve o limite de conexões por IP. Um item do pool escolhido tem prioridade sobre o proxy próprio. A URL do proxy é guardada criptografada. Na aba Proxies dá para definir por conta, distribuir uma lista em lote e testar o IP de saída de cada conta conectada. **Não troque o user agent da sessão:** um UA de Chrome "falso" (sem os client hints correspondentes) faz o Cloudflare desconfiar e servir o desafio cheio; o UA padrão do Electron entra direto.
...
Tema **"Universe"** (redesign de 23/09/2026, validado no canvas de design antes de entrar no código): espaço profundo com um toque de índigo, superfícies em vidro (`.glass`, `.glass-strong`), fio de 1 px, e uma só cor viva, o **pulsar** (`accent`), reservada ao que está acontecendo agora (online, agente ativo, addon ligado). Branco ("luz estelar") é a ação principal; **solar** (`warn`) pede atenção; **nova** (`danger`) é falha. Tokens no `tailwind.config.js` com os MESMOS nomes do tema anterior (`bg`, `surface`, `raised`, `line`, `fg`, `muted`, `dim`, `accent`, `info`, `success`, `warn`, `danger`, `violet`), então todo componente antigo continua válido; nunca hex solto nos componentes. Fontes Manrope Variable (interface), Space Grotesk Variable (títulos e wordmark) e JetBrains Mono (dados), via `@fontsource`.

**O céu** (`src/components/Sky.tsx`) fica atrás do jogo e na abertura: estrelas determinísticas que cintilam fora de fase e duas nebulosas quase invisíveis que derivam devagar. **A marca** (`src/components/Brand.tsx`, CSS em `index.css`) é um núcleo, uma órbita e um satélite em movimento: o app orbita o jogo. Movimento sempre lento (órbita 14 s, anel de avatar 40 s, nebulosa 46 a 58 s, entrada de folhas 480 ms) e tudo desliga com "reduzir movimento" do sistema.

Layout: barra de título (marca e wordmark, navegação em pílula no centro: Jogo, Multidão, Sala, Protocolo, Logs, Addons; endereço à direita), **palco** com o jogo em tela cheia e cantos arredondados, **ilha** flutuante no topo (sua conta, sala e pedidos pendentes), leitura do Add User All no canto, **dock** que recolhido é uma barra fina e aberto é uma folha em vidro que sobe sobre o jogo (Logs, Board, Sala, Protocolo, Multidão; redimensionável), e **gaveta** lateral de addons em vidro. O kit de peças (`src/components/ui.tsx`) é em pílula: botão primário branco, secundário em fio, interruptor estilo iOS.

**Diagnóstico visual.** `HABBLET_SCREENSHOT=<arquivo.png>` (com `HABBLET_SCREENSHOT_DELAY_MS`, padrão 5000) faz a instância tirar uma captura da janela e fechar; junto com `HABBLET_USER_DATA` e `HABBLET_OPEN`, serve para conferir uma tela sem tocar na sessão real.

Atalhos: **Ctrl+K** abre a **paleta** (`src/components/Palette.tsx`: um campo para endereço, ir para uma aba, ligar ou configurar um addon, mandar um comando à Multidão com a mesma gramática da barra de comando, e exportar/importar perfil; setas escolhem, Enter executa, Esc fecha) · **Ctrl+J** alterna o dock · **Ctrl+,** abre/fecha os addons. Funcionam mesmo com o foco dentro do jogo.

**Logs.** Uma linha do tempo só, com tudo o que o app registra (motores dos addons, Multidão por conta, Colar quarto, sistema), filtrável por origem (chips com contagem; a origem é lida do prefixo entre colchetes, `src/components/logSource.ts`) e por texto (nick, quarto, palavra). Exportar leva o que está filtrado.
